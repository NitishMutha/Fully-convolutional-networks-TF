...
class FCN8VGG:
    def __init__(self, enableTensorboard=False, vgg16_npy_path=None):

        if vgg16_npy_path is not None:
            self.data_dict = np.load(vgg16_npy_path, encoding='latin1').item()
        self.wd = 5e-4
        self.epsilon = tf.constant(value=1e-4)
        self.enableTensorboard = enableTensorboard
        print("npy file loaded!")

    def build(self, rgb, keepProb, num_classes=151, random_init_fc8=False,
              debug=False):
        """
        ...
        self.upscore32_1 = tf.nn.relu(self.upscore32)
        self.upscore32_pred = self._score_layer(self.upscore32_1, "upscore32_pred",
                                                num_classes=num_classes, in_features=num_classes,
                                                stddev=0.001)

        self.logits = tf.reshape(self.upscore32_pred, (-1, num_classes))
        ...
        if self.enableTensorboard:
            ...
            tf.summary.image('Foreground Prob', tf.expand_dims(self.probabilities[:, :, :, 1] * 255, -1),
                             max_outputs=10)
